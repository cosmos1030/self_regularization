...
# 훈련 함수 (시드별)
# ---------------------------
def run_training(seed, epochs=100, batch_size=64, learning_rate=0.0001, optimizer_type="adam", noise=None, save_path=None):
    # 시드 고정
    ...
    base_dir = os.path.join("runs", f"alex_seed{seed}_batch{batch_size}_{optimizer_type.lower()}_lr{learning_rate}_epochs{epochs}_noise{noise}")
...
def main():
    ...
    noise_ratio = 1  # Set noise level (match the one used when generating noisy labels) 0~1
    noise_seed = 5
    ...
    for seed in range(start_seed, total_seeds + 1):
        ...
        train_acc, test_acc, base_dir = run_training(seed, epochs=epochs, batch_size=batch_size,
                                                      learning_rate=learning_rate, optimizer_type=optimizer_type, noise=noise_ratio*100, save_path=save_path)
