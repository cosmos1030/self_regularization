...
def get_data_loaders(batch_size=64, save_path=None):
    ...
    # **Load Pre-Generated Noisy Labels Instead of Creating New Ones**
...
# 훈련 함수 (시드별)
# ---------------------------
def run_training(seed, epochs=100, batch_size=64, learning_rate=0.0001, optimizer_type="adam", save_path=None):
    # 시드 고정
    ...
    base_dir = os.path.join("runs", f"alex_seed{seed}_batch{batch_size}_{optimizer_type.lower()}_lr{learning_rate}_epochs{epochs}")
...
# ---------------------------
def gpca_analysis(eigen_csv, gpca_dim=1, max_iter=200000, tolerance=1e-5, patience=100, device=torch.device("cpu")):
...
def update_summary_csv(output_file, row, seed):
    if os.path.exists(output_file):
        ...
        df = pd.DataFrame(columns=['seed','train_acc','test_acc','S1_fit','S1_RSS','S2_fit','S2_RSS'])
...
def main():
    ...
    # setting_name 자동 생성
    setting_name = f"alex_batch{batch_size}_{optimizer_type.lower()}_lr{learning_rate}_epochs{epochs}_noise{noise_ratio*100}"
    output_dir = os.path.join("multiseed_output", setting_name)
    ...
    for seed in range(start_seed, total_seeds + 1):
        ...
        train_acc, test_acc, base_dir = run_training(seed, epochs=epochs, batch_size=batch_size,
                                                      learning_rate=learning_rate, optimizer_type=optimizer_type, save_path=save_path)
