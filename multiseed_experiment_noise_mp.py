...
def get_data_loaders(batch_size=64, save_path=None):
    ...
    # 이미 만들어 둔 noisy label 파일이 save_path에 있다고 가정
    if os.path.exists(save_path):
        print(f"Loading noisy labels from {save_path}")
        trainset.targets = np.load(save_path).tolist()
    else:
        raise FileNotFoundError(f"Error: Noisy label file {save_path} not found! Please generate it first.")
    trainloader = DataLoader(trainset, batch_size=batch_size, shuffle=True, num_workers=2)
    testloader = DataLoader(testset, batch_size=batch_size, shuffle=False, num_workers=2)
    return trainloader, testloader

# ---------------------------
# MP 분포 기반 spike count 계산 함수
# ---------------------------
def compute_spike_count(weight: np.ndarray):
    """
    weight: (out_features, in_features) numpy 배열
    return: (spike_count, lambda_plus)
      - spike_count: MP 분포 상단경계(lambda_plus)를 초과하는 eigenvalue(= singular value^2 / n)의 개수
      - lambda_plus: MP 분포 upper bound
    """
...
# 훈련 함수 (시드별)
# ---------------------------
def run_training(seed, epochs=100, batch_size=64, learning_rate=0.0001, optimizer_type="adam", noise_ratio=None, save_path=None):
    # 시드 고정
    torch.manual_seed(seed)
    np.random.seed(seed)
    
    trainloader, testloader = get_data_loaders(batch_size, save_path=save_path)
    model = miniAlexNet()
    ...
    # fc1, fc2, fc3 각각의 earliest spike epoch 기록용
    earliest_spike_epoch = [None, None, None]  # index: 0->fc1, 1->fc2, 2->fc3
    ...
    for epoch in range(epochs):
        ...
        # 에폭 종료 후 – 모델의 모든 Linear layer에 대해 leading eigenvector 저장 & spike 카운트 체크
        layer_idx = 0
        for layer in model.classifier:
            if isinstance(layer, nn.Linear):
                ...
                file_path = os.path.join(eigen_dir, f"fc{layer_idx+1}.csv")
                with open(file_path, "a") as f:
                    np.savetxt(f, leading_eigenvector.reshape(1, -1))
                
                # spike count 계산
                spike_count, _ = compute_spike_count(weight)
                # 아직 spike epoch이 기록 안 되었고 spike_count >= 1 이면 기록
                if earliest_spike_epoch[layer_idx] is None and spike_count >= 1:
                    earliest_spike_epoch[layer_idx] = epoch + 1
                layer_idx += 1
    ...
    return train_acc, test_acc, base_dir, earliest_spike_epoch

# ---------------------------
# GPCA 분석 함수 (피팅 및 RSS 측정)
# ---------------------------
def gpca_analysis(eigen_csv, gpca_dim=1, max_iter=200000, tolerance=1e-5, patience=100,
                  device=torch.device("cpu"), start_epoch=1, end_epoch=None):
    ...
    from tqdm import tqdm
    with tqdm(range(max_iter), desc=f"GPCA-{gpca_dim}D Fitting (epoch range {start_epoch}~{end_epoch})", leave=False) as pbar:
        for step in pbar:
    ...
    # 1D GPCA인 경우 지오데식 평균까지 고려하여 fitting score 계산
    if gpca_dim == 1:
...
def update_summary_csv(output_file, row, seed):
    ...
    CSV 열: 
    [
      'seed','train_acc','test_acc',
      'S1_fit','S1_RSS','S2_fit','S2_RSS',
      'start_epoch',
      'S1_fit_selected','S1_RSS_selected','S2_fit_selected','S2_RSS_selected'
    ]
    """
    desired_cols = [
        'seed','train_acc','test_acc',
...
def main():
    ...
    noise_ratio = 1  # 노이즈 비율 0~1
    noise_seed = 5
    save_path = f"./data/noisy_labels{int(noise_ratio * 100)}_seed{noise_seed}.npy"
    
    # setting_name 자동 생성
    setting_name = f"alex_batch{batch_size}_{optimizer_type.lower()}_lr{learning_rate}_epochs{epochs}_noise{int(noise_ratio*100)}"
    output_dir = os.path.join("multiseed_output_test", setting_name)
    os.makedirs(output_dir, exist_ok=True)
    
    # fc1.csv 파일을 기준으로 이미 처리된 seed 파악
    ...
    for seed in range(start_seed, total_seeds + 1):
        ...
        train_acc, test_acc, base_dir, earliest_spike_epoch = run_training(
            seed, epochs=epochs, batch_size=batch_size, learning_rate=learning_rate,
            optimizer_type=optimizer_type, noise_ratio=int(noise_ratio*100), save_path=save_path
        )
        print(f"Seed {seed}: train_acc={train_acc:.4f}, test_acc={test_acc:.4f}")

        # fc 레이어별 결과 (fc1, fc2, fc3)
        for fc_idx in range(1, 4):
            eigen_csv = os.path.join(base_dir, "eigenvectors", f"fc{fc_idx}.csv")
            print(f"GPCA for fc{fc_idx} ...")

            # 1) 전체 에폭(1~100) 구간 GPCA
            S1_fit, S1_RSS = gpca_analysis(eigen_csv, gpca_dim=1, device=device)
            S2_fit, S2_RSS = gpca_analysis(eigen_csv, gpca_dim=2, device=device)
            ...
            # 2) 스파이크 발생 epoch에 대한 GPCA
            spk_epoch = earliest_spike_epoch[fc_idx - 1]  # fc_idx:1→0, 2→1, 3→2
            if spk_epoch is None:
                # 전 구간에 걸쳐 spike 없음
                row["start_epoch"] = np.nan
                row["S1_fit_selected"] = np.nan
                row["S1_RSS_selected"] = np.nan
                row["S2_fit_selected"] = np.nan
                row["S2_RSS_selected"] = np.nan
            else:
                # spk_epoch ~ 100 구간 GPCA
                row["start_epoch"] = spk_epoch
                S1_fit_sel, S1_RSS_sel = gpca_analysis(eigen_csv, gpca_dim=1, device=device, start_epoch=spk_epoch)
                S2_fit_sel, S2_RSS_sel = gpca_analysis(eigen_csv, gpca_dim=2, device=device, start_epoch=spk_epoch)
                row["S1_fit_selected"] = S1_fit_sel
                row["S1_RSS_selected"] = S1_RSS_sel
                row["S2_fit_selected"] = S2_fit_sel
                row["S2_RSS_selected"] = S2_RSS_sel
            
            # CSV 업데이트
            output_file = os.path.join(output_dir, f"fc{fc_idx}.csv")
