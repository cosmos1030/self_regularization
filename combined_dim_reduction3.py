...
def main():
    ...
    for base_dir in args.dirs:
        for layer in args.layers:
            ...
            for t in range(num_epochs_to_process): # 0부터 (len-1)까지의 인덱스 (aligned_all_epochs_Vh 기준)
                ...
                if t < num_epochs_to_process - 1: # 마지막 epoch이 아니라면 다음 epoch을 고려
                    ...
                    if Vh_t_plus_1 is None:
                        ...
                        # current_tracked_idx는 변경 없이 다음 루프로
                        continue

                    v_current_at_t = vector_to_add # Vh_t[current_tracked_idx]
                    norm_v_current_at_t = np.linalg.norm(v_current_at_t)

                    if norm_v_current_at_t < 1e-9: # 현재 벡터가 거의 제로 벡터면 유사도 비교 무의미
                        print(f"Warning: Tracked vector at epoch {args.start_epoch+t} (idx {current_tracked_idx}) is near zero. Assuming no change in index.")
                        crossing_info_details.append(
                            (args.start_epoch + t, args.start_epoch + t + 1, current_tracked_idx, current_tracked_idx, "N/A (Current vector is zero)")
                        )
                        continue # current_tracked_idx 유지

                    # 비교 대상 인덱스 후보: current_tracked_idx-1, current_tracked_idx, current_tracked_idx+1
                    candidate_next_indices = []
                    if current_tracked_idx > 0:
                        candidate_next_indices.append(current_tracked_idx - 1)
                    candidate_next_indices.append(current_tracked_idx)
                    if current_tracked_idx < max_sv_idx_overall: # Vh_t_plus_1.shape[0] - 1 와 동일
                        candidate_next_indices.append(current_tracked_idx + 1)
                    
                    best_next_idx = current_tracked_idx # 기본값은 현재 인덱스 유지
                    max_similarity = -1.0

                    for next_idx_candidate in candidate_next_indices:
                        # Vh_t_plus_1의 유효 인덱스인지 확인
                        if not (0 <= next_idx_candidate < Vh_t_plus_1.shape[0]):
                            continue 
                        
                        v_candidate_at_t_plus_1 = Vh_t_plus_1[next_idx_candidate]
                        norm_v_candidate_at_t_plus_1 = np.linalg.norm(v_candidate_at_t_plus_1)

                        if norm_v_candidate_at_t_plus_1 < 1e-9: # 후보 벡터가 제로 벡터면 스킵
                            continue
                        
                        # 코사인 유사도 (aligned_all_epochs_Vh는 이미 부호 정렬됨)
                        # np.abs는 혹시 모를 미세한 부호 차이나, crossing 정의에 따라 필요할 수 있음
                        similarity = np.dot(v_current_at_t, v_candidate_at_t_plus_1) / (norm_v_current_at_t * norm_v_candidate_at_t_plus_1)
                    ...
                    current_tracked_idx = best_next_idx # 다음 스텝에서 추적할 인덱스 업데이트
            ...
            tolerance_loss_change = 1e-6 # 손실 변화량 기준
