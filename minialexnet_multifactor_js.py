...
base_dir = f"runs/alex_seed{seed}_batch{batch_size}_{optimizer_type}_lr{learning_rate}_epochs{epochs}_jse"
...
trainloader=torch.utils.data.DataLoader(
    torchvision.datasets.CIFAR10("./data",True ,tfm,download=True),
    batch_size=batch_size,shuffle=True)
testloader =torch.utils.data.DataLoader(
    torchvision.datasets.CIFAR10("./data",False,tfm,download=True),
...
def js_vec(W_cpu,k):                         # vector-only
    W=W_cpu.to(device)
    U,S,Vh=torch.linalg.svd(W,full_matrices=False)
    ...
    Hjs=H@C+M@(torch.eye(k,device=device)-C)
    Ujs,_,_=torch.linalg.svd(Hjs,full_matrices=False)
    return Ujs[:,:k] @ torch.diag(S_k) @ Vh_k

def js_full(W_cpu,k):                        # vector+σ
    W=W_cpu.to(device)
    U,S,Vh=torch.linalg.svd(W,full_matrices=False)
    U_k,S_k,Vh_k=U[:,:k],S[:k],Vh[:k]
    n=W.size(0); H=U_k*(S_k/np.sqrt(n)).reshape(1,-1)
    ...
    Ujs,_,_=torch.linalg.svd(Hjs,full_matrices=False)
    return Ujs[:,:k] @ torch.diag(sigma_js) @ Vh_k
...
for ep in range(1,epochs+1):
    ...
    for idx,(name,fc_idx) in enumerate(fc_indices.items(),start=1):
        ...
        n,p=W.shape; spec=(S**2)/n
        mp=np.var(W)*(1+np.sqrt(p/n))**2
        plt.figure(figsize=(6,4))
        plt.hist(spec,bins=300,density=True,alpha=.6); plt.axvline(mp,color='r',label='MP+')
        plt.title(f"FC{idx} Spectrum at Epoch {ep}"); plt.legend()
        plt.savefig(f"{spectrum_dir}/fc{idx}_spectrum_epoch{ep}.png"); plt.close()

    # ----- Reconstruction -----
    if ep in recon_epochs:
        saved={i:model.classifier[i].weight.data.clone().cpu() for i in fc_indices.values()}
        for name,idx in fc_indices.items():
            raw,vec,ful=[],[],[]
            for k in tqdm(range(1,max_rank[name]+1),desc=f"[Recon {name}] Ep{ep}",leave=False):
                # baseline(top-k)
...
########################################################################
# 8. Overlay Graphs (baseline / JS-vec / JS-full)
########################################################################
