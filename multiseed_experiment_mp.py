...
# ---------------------------
# Marchenko-Pastur(MP) 기반 spike count 계산 함수
# ---------------------------
def compute_spike_count(weight: np.ndarray):
    """
    weight: (out_features, in_features) 형태의 numpy 배열
    return: (spike_count, lambda_plus)
      - spike_count: MP 분포 상단경계(lambda_plus)보다 큰 eigenvalue(= singular value^2 / n)의 개수
      - lambda_plus: MP 분포에서 이론적 upper bound
    """
    # SVD
    U, S, Vh = np.linalg.svd(weight, full_matrices=False)
    n, p = weight.shape
    # Empirical spectrum: s^2 / n
    spec = np.square(S) / n
    gamma = p / n
...
def run_training(seed, epochs=100, batch_size=64, learning_rate=0.0001, optimizer_type="adam"):
    ...
    # -------------------------------
    # 스파이크 발생 시점을 저장할 리스트
    # fc1, fc2, fc3 각각 earliest 스파이크 epoch을 기록
    # 아직 spike가 나타나지 않았으면 None
    # -------------------------------
    ...
    for epoch in range(epochs):
        ...
        # 에폭 종료 후 – 모델의 classifier 내 각 Linear layer에 대해
        # 1) leading eigenvector 저장
        # 2) spike count 계산 → earliest_spike_epoch 업데이트
        layer_index = 0
        for layer in model.classifier:
            if isinstance(layer, nn.Linear):
                ...
                # leading eigenvector 저장
                file_path = os.path.join(eigen_dir, f"fc{layer_index+1}.csv")
                with open(file_path, "a") as f:
                    np.savetxt(f, leading_eigenvector.reshape(1, -1))
                
                # MP Upper Bound와 비교하여 spike 확인
                spike_count, _ = compute_spike_count(weight)
                
                # 아직 earliest_spike_epoch[layer_index]가 None이면, spike_count >= 1인지 확인
                if earliest_spike_epoch[layer_index] is None and spike_count >= 1:
                    earliest_spike_epoch[layer_index] = epoch + 1  # 1-based index
...
def gpca_analysis(eigen_csv, gpca_dim=1, max_iter=200000, tolerance=1e-5, patience=100,
    ...
    # start_epoch ~ end_epoch 범위의 eigenvector data
    ...
    if gpca_dim == 1:
        ...
        def function_to_minimize(t_arr, param_arr):
            # distmat[i, j] = (dist on sphere)^2 between angle t_arr[i] and param_arr[j]
...
def update_summary_csv(output_file, row, seed):
    """
    기존 CSV에 seed가 같은 행이 있으면 제거하고, row를 추가.
    """
...
def main():
    ...
    for seed in range(start_seed, total_seeds + 1):
        ...
        for layer_index in range(1, 4):
            ...
            # 2) MP spiking epoch에 대한 GPCA
            # earliest_spike_epoch[layer_index-1] -> 이 fc 레이어에서 spike>=1 최초 발생 epoch
            spk_epoch = earliest_spike_epoch[layer_index-1]
            if spk_epoch is None:
                # 만약 전 epoch동안 spike가 없었다면, (예: None)
                # 편의상 101처럼 범위를 벗어난 값으로 처리하거나, "no_spike" 라고 둘 수도 있음
