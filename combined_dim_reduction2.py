...
def main():
    ...
    # 1) Argument Parsing
    # ------------------------------------------------------------------------------
    parser = argparse.ArgumentParser(description="Project eigenvectors…")
    ...
    parser.add_argument('--eig-index',  type=int, default=0, help="Which singular vector (row of Vh) to track (0-based)")
    ...
    for base_dir in args.dirs:
        for layer in args.layers:
            ...
            # 파일 경로 세팅
            # -------------------------------------------
            # No specific input CSV file path based on eig-index for this script as it reads from NPZs
            # csv_file = os.path.join(base_dir, f'eigenvectors/{layer}.csv') # This line seems unused now
            epoch_range_str = f"ep{args.start_epoch}-{args.end_epoch}"
            eig_index_str = f"eig{args.eig_index}" # String for eig-index
            sub_dir = f"eig{args.eig_index}_ep{args.start_epoch}-{args.end_epoch}"
            output_folder = os.path.join(base_dir, sub_dir)
            os.makedirs(output_folder, exist_ok=True) 

            # 결과 파일(공통)
            # Added {eig_index_str} to filenames
            output_csv = os.path.join(base_dir, sub_dir, f"{layer}_sphere_{eig_index_str}_{epoch_range_str}.csv")
            output_heatmap_fig = os.path.join(base_dir,sub_dir, f"{layer}_sphere_heatmap_{eig_index_str}_{epoch_range_str}.png")
            output_residual_heatmap_fig = os.path.join(base_dir,sub_dir, f"{layer}_sphere_residual_heatmap_{eig_index_str}_{epoch_range_str}.png")
            # 1D GPCA 전용 그래프
            output_1d_fig = os.path.join(base_dir,sub_dir, f"{layer}_1Dgeodesic_{eig_index_str}_{epoch_range_str}.png")
            output_1d_residual_fig = os.path.join(base_dir,sub_dir, f"{layer}_residual_vs_epoch_1D_{eig_index_str}_{epoch_range_str}.png") # Added for 1D residual plot
            # 2D GPCA 전용 그래프 (for residual vs epoch plot)
            output_2d_residual_fig = os.path.join(base_dir,sub_dir, f"{layer}_residual_vs_epoch_2D_{eig_index_str}_{epoch_range_str}.png") # Added for 2D residual plot


            # print(f"CSV file: {csv_file}") # This can be removed or commented if csv_file isn't used as input


            # 1) NPZ에서 고유벡터 읽기 (선택한 구간, 지정한 eig-index)
            vectors = []
            for ep in range(args.start_epoch, args.end_epoch+1):
                npz_path = os.path.join(base_dir, f"eigenvectors/epoch{str(ep)}.npz")
                if not os.path.exists(npz_path):
                    continue
                data = np.load(npz_path)
                # each layer npz field is e.g. "fc1_Vh"
                Vh = data[f"{layer}_Vh"]            # shape (num_svs, dim)
                k  = args.eig_index
                if k < 0 or k >= Vh.shape[0]:
                    raise IndexError(f"eig-index {k} out of range for {layer} (max {Vh.shape[0]-1})")
                vectors.append(Vh[k])                # pick the k-th singular vector

            if len(vectors) == 0:
                print(f"❌ No .npz files found for {layer}. Skipping.")
                continue

            eigv = np.stack(vectors, axis=0)  # shape (num_epochs, dim)
            l, p = eigv.shape
            print(f"Loaded {l} vectors of dimension {p} for {layer}")
