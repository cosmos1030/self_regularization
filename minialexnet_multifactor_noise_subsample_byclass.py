...
recon_epochs  = []            # reconstruction을 할 epoch 리스트
optimizer_type= 'sgd'
...
base_dir = (f"runs/alex_seed{seed}_batch{batch_size}_{optimizer_type}"
            ...
            f"_noise{int(noise_ratio*100)}_seed{noise_seed}_sub{int(train_percent*100)}_jse_eigen")
...
def subsample_by_class(dataset, percent: float, seed_: int):
    ...
    for idx, lab in enumerate(dataset.targets):
        cls_indices[lab].append(idx)
    keep = []
...
# testloader (전체 10클래스 평가용)
testloader = torch.utils.data.DataLoader(
    testset, batch_size=batch_size, shuffle=False)

# 클래스별 인덱스 맵핑 (학습 중 Subset 생성용)
cls_indices = defaultdict(list)
for idx, lab in enumerate(trainset.targets):
    cls_indices[lab].append(idx)
...
def js_vec(W_cpu, k):
    ...
    p = H.size(0)
    e = torch.ones(p,1,device=device)
    M = (e @ (e.T @ H)) / p
    R = H - M
...
for ep in range(1, epochs+1):
    # --- (A) 활성 클래스 결정 ---
    if ep <= 50:
        active = set(range(5))    # classes 0–4
    else:
        active = set(range(5,10)) # classes 5–9

    # --- (B) Subset DataLoader 생성 ---
    keep = [i for c in active for i in cls_indices[c]]
    sub_ds = Subset(trainset, keep)
    trainloader = torch.utils.data.DataLoader(sub_ds,
                        batch_size=batch_size, shuffle=True)

    # --- (C) Training ---
    model.train()
    for x,y in tqdm(trainloader, desc=f"[Train] Ep{ep}", leave=False):
        x,y = x.to(device), y.to(device)
        optimizer.zero_grad()
    ...
    with torch.no_grad():
        ...
        for layer in model.classifier:
            if isinstance(layer, nn.Linear):
                ...
                lead_ev = Vh[0,:]
                with open(os.path.join(eigenvector_dir,f"fc{fc_idx_dump}.csv"),"a") as f:
                    np.savetxt(f, lead_ev.reshape(1,-1))
                with open(os.path.join(bias_dir,f"fc{fc_idx_dump}_bias.csv"),"a") as f:
                    np.savetxt(f, b.reshape(1,-1))
                fc_idx_dump += 1

    # --- (F) Reconstruction (recon_epochs에 있을 때만) ---
    if ep in recon_epochs:
        saved = {i: model.classifier[i].weight.data.cpu().clone()
                 for i in fc_indices.values()}
        for name,idx in fc_indices.items():
            raw, vec, ful = [], [], []
            r_max = max_rank[name]
            for k in tqdm(range(1, r_max+1),
                          desc=f"[Recon {name}] Ep{ep}", leave=False):
                # top-k
                model.classifier[idx].weight.data = topk_recon(saved[idx],k).to(device)
                raw.append(accuracy(model, testloader))
                # JS-vector / JS-full
                if k < r_max:
